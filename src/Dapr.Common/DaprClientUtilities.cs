...
internal static class DaprClientUtilities
{
    ...
    internal static void AddTraceContextHeaders(Metadata headers)
    {
        ...
        if (!headers.Any(header => string.Equals(header.Key, TraceParentHeader, StringComparison.OrdinalIgnoreCase)))
        {
        ...
        {
            if (!headers.Any(header => string.Equals(header.Key, TraceStateHeader, StringComparison.OrdinalIgnoreCase)))
            {
