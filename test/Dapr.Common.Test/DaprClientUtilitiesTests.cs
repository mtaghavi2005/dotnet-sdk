...
public sealed class DaprClientUtilitiesTests
{
    ...
    [Fact]
    public void AddTraceContextHeaders_ShouldNotDuplicateExistingHeaders()
    {
        var previous = Activity.Current;
        using var activity = new Activity("parent");
        activity.SetIdFormat(ActivityIdFormat.W3C);
        ...
        try
        {
            ...
            Assert.Equal(1, headers.Count(header => header.Key == "traceparent"));
            Assert.Equal(1, headers.Count(header => header.Key == "tracestate"));
            Assert.Equal(1, headers.Count(header => header.Key == "grpc-trace-bin"));
            Assert.True(TryGetHeader(headers, "traceparent", out var traceParent));
            Assert.Equal("existing-traceparent", traceParent);
            Assert.True(TryGetHeader(headers, "tracestate", out var traceState));
            Assert.Equal("existing-tracestate", traceState);
